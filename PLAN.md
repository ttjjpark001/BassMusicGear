...
완료 후 git add/commit/push. 메시지: "feat(phase7): preset system — manager, 15 factory presets, A/B slots, PresetPanel, EQ user presets"
```
...
> 📝 **Phase 8 시작 전 검토 사항 — Pre-FX 순서 변경 기능**
> 현재 Pre-FX(Overdrive → Octaver → EnvelopeFilter) 순서는 SignalChain.cpp에 하드코딩되어 있음.
> Phase 8에서 SignalChainView를 구현할 때 드래그로 블록 순서를 바꾸는 기능을 추가할지 검토할 것.
...
- **[Phase 4 이월]** Overdrive JFET 타입 전환 → Tube와 다른 음색 (그릿+클린 혼합) 확인 (od_type ComboBox UI 구현 후 확인)
- **[Phase 4 이월]** Overdrive Fuzz 타입 전환 → 매우 강한 하드 클리핑, 사각파에 가까운 음색 확인 (od_type ComboBox UI 구현 후 확인)
- **[Phase 3→4 이월]** Compressor 노브 우클릭 → 기본값 리셋 동작 확인 (Compressor EffectBlock UI 구현 후 확인)
