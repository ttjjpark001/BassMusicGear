...
아래 두 옵션 중 하나를 선택해야 한다. **결정 전에 PLAN.md를 검토하여 각 Phase의 작업량과 충돌 여부를 확인할 것.**

| 옵션 | Phase | 이유 |
|------|-------|------|
| **옵션 A** | Phase 6 (Bi-Amp + DI Blend) | DSP 작업 집중 Phase. AmpVoicing도 DSP 모듈이므로 함께 처리하면 신호 체인 구조를 한 번에 정리 가능 |
| **옵션 B** | Phase 9 (릴리즈 준비) | 실제 IR 교체와 동시에 Voicing을 적용하면 최종 음색을 한 번에 튜닝 가능. 단, Phase 9는 이미 작업량이 많음 |

> **권장**: 옵션 A (Phase 6). Voicing 없이는 Pre-FX(Phase 4), Post-FX(Phase 5) 단계에서도 앰프 간 음색이 구분되지 않아 테스트 신뢰도가 낮아진다. 빠를수록 이후 Phase 테스트 품질이 향상된다.
