...
미확인 항목. SSD 여유 공간 확인 권장.

| 용도 | 필요 용량 | 비고 |
|---|---|---|
| JUCE 소스 (git submodule) | ~500MB | |
| 빌드 디렉터리 (Debug + Release) | ~5–8GB | JUCE 빌드 결과물이 큼 |
| Visual Studio 2022 (Windows) | ~10GB | |
| Xcode (macOS) | ~12GB | |
| IR 파일 (Resources/IR/) | ~50–200MB | WAV 파일 수에 따라 |
| 팩토리 프리셋 XML | ~수MB | |
| NAM 모델 파일 (Post-MVP) | ~10–500MB/모델 | 모델 종류에 따라 다양 |
| **합계 권장 여유 공간** | **50GB 이상** | **SSD 필수** (HDD는 JUCE 빌드 속도 심각하게 저하) |
...
| SSD 여유 | ❓ **확인 필요** | 50GB 이상 여유 공간 확보 권장 |
...
- [ ] Xcode 최신 버전 설치 (`xcode-select --install`)
