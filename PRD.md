...
- **DI Blend**: 클린 DI 신호와 프로세스드 신호의 혼합 비율 (0–100%)
  - **프로세스드**: Pre-FX → 프리앰프 → 톤스택 → Post-FX → 파워앰프 → 캐비닛 IR 을 모두 통과한 신호
  - 베이스 앰프 시뮬레이터의 핵심 컨트롤. 저역 펀치와 어택은 클린 DI에서, 앰프 캐릭터와 하모닉 컬러는 프로세스드에서 가져오는 것이 일반적인 사용 패턴.
- **DI Source** (2-way 선택):
  - **Post Gate** (기본): 노이즈 게이트·튜너 통과 후 분기. 게이트가 자른 노이즈는 클린 DI에도 반영되지 않음. 실용적이며 가장 일반적인 방식.
  - **Pre Everything**: 입력 직후 분기. 게이트·튜너 포함 어떤 처리도 없는 완전한 원본 신호.
