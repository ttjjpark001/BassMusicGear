...
- [ ] 오디오 인터페이스 베이스 입력 → 처리된 소리 출력 (수동 확인 필요)
- [ ] 앱 종료 → 크래시 없음 (수동 확인 필요)

### 튜너 (YIN 피치 트래킹)
- [ ] E1(41Hz) 연주 → 튜너 디스플레이에 "E" 음이름 표시 (수동 확인 필요)
- [ ] E1 연주 중 센트 편차 바가 움직임 (수동 확인 필요)
- [ ] 다른 음(A, D, G) 연주 → 각각 "A", "D", "G" 표시 (수동 확인 필요)
- [ ] 오픈 현 5개(B0/E1/A1/D2/G2) 연주 → 각 음이름 정확히 표시 (수동 확인 필요)
- [ ] Mute ON 버튼 클릭 → 오디오 출력이 무음으로 전환 (수동 확인 필요)
- [ ] Mute OFF 버튼 클릭 → 오디오 출력 복귀 (수동 확인 필요)
- [ ] 참조 주파수 A=445Hz로 변경 → 센트 편차 수치 변화 확인 (수동 확인 필요)
- [ ] 무음 상태에서 튜너 디스플레이 상태 (음이름 표시 안정적이거나 "--" 표시) (수동 확인 필요)
...
### 신호 체인 전체 동작
- [ ] NoiseGate Threshold 최대 → 완전 무음 (수동 확인 필요)
- [ ] NoiseGate OFF → 신호 통과 (수동 확인 필요)
- [ ] 5종 앰프 모델 전환 → 각 모델별 음색 차이 확인 (수동 확인 필요)

### UI / 레이아웃
- [ ] 창 리사이즈 → 레이아웃 비율 유지 (수동 확인 필요)
- [ ] 컴프레서 파라미터 노브 우클릭 → 기본값 리셋 동작 (수동 확인 필요)
