                 ...
                 └─ Bi-Amp 크로스오버 (ON/OFF, 기준 주파수 가변)
                      ...
                      └─ HP (고역) 또는 [OFF 시] 전대역                           │
                           └─ Pre-FX (오버드라이브, 옥타버, 엔벨로프 필터)         │
                                └─ 프리앰프 (Gain, 캐릭터)                        │
                                     └─ [Amp Voicing 필터] (자동 적용)           │
                                          └─ 톤스택 EQ                           │
                                          └─ 그래픽 EQ (10밴드)                  │
                                               └─ Post-FX / FX Loop             │
                                                    └─ 파워앰프                  │
                                                    [Pre-IR 선택 시] DI Blend ◄──┤
                                                         └─ 캐비닛 IR            │
                                                    [Post-IR 선택 시] DI Blend ◄─┘
                                                              └─ 출력 레벨
```
...
- **고유한 Amp Voicing 필터** — Preamp 출력 직후 ToneStack 앞에 자동 적용되는 고정 바이쿼드 필터 체인. EQ 노브와 무관하게 각 앰프의 회로 고유 주파수 특성을 재현. 세부 값은 `AmpVoicingPlan.md` 참조
...
- **Volume**: 프리앰프 출력 레벨
- **Amp Voicing 필터** (사용자 노브 없음): 앰프 모델 선택 시 자동 적용되는 고정 필터. Preamp 출력과 ToneStack 사이에 위치하며 앰프 고유의 주파수 성격을 확립. 예: American Vintage는 80Hz 저역 강조 + 1.5kHz 약한 컷, British Stack은 60Hz 이하 HP컷 + 500Hz 미드 부스트
- **톤스택**: 앰프 모델별 EQ
...
**파워앰프 컨트롤** (앰프 모델 공통):
- **Drive**: 파워앰프 입력 드라이브 레벨 — 출력 새추레이션 양 결정
- **Presence**: 파워앰프 NFB(음부환류) 루프 고역 보정 노브 — 공기감·밝기 조정
- **Sag**: 파워 튜브 플레이트 전압 새깅 시뮬레이션 — 다이나믹 응답감 조절
  - 튜브 앰프 모델(American Vintage / Tweed Bass / British Stack)에서만 활성화
  - Modern Micro / Italian Clean (JFET / Class D)에서는 비활성화
...
- 내장 IR 프리셋 (앰프 모델당 최소 2종: close/dark)
  ...
  - 1x15 빈티지 (리본 마이크)
  - 2x12 British (SM57)
  - 2x10 Modern (Condenser 클로즈) — Italian Clean 기본 매칭
...
튜너 직후, Bi-Amp 크로스오버 앞에 고정 배치. 드라이브/왜곡 전에 다이나믹을 먼저 정리하는 표준 순서.
...
클린 DI 경로와 프로세스드 경로를 혼합하는 블록. 캐비닛 IR 전후 중 위치를 선택할 수 있다.

**두 경로 정의:**
- **클린 DI**: Bi-Amp 크로스오버 직후 분기. Bi-Amp ON 시 LP(저역)만, OFF 시 전대역
- **프로세스드**: IR Position 설정에 따라 범위가 다름
  - **Post-IR 선택 시**: Pre-FX → 프리앰프 → 톤스택 → 그래픽 EQ → Post-FX → 파워앰프 → **캐비닛 IR**까지 포함한 신호 (DI Blend는 캐비닛 이후에 위치)
  - **Pre-IR 선택 시**: Pre-FX → 프리앰프 → 톤스택 → 그래픽 EQ → Post-FX → 파워앰프까지만 통과한 신호 (캐비닛 IR은 DI Blend 결과 전체에 일괄 적용)

**컨트롤:**
- **Enabled (ON/OFF)**: DI Blend 블록 활성화 토글
  - **OFF**: 클린 DI 혼합 비활성, 프로세스드 신호만 출력 (앰프 체인 통과 결과만). 다른 컨트롤은 모두 비활성화되어 보인다.
  - **ON**: 아래 Blend/Clean Level/Processed Level/IR Position 컨트롤이 활성화되어 두 경로를 혼합한다.
- **Blend**: 혼합 비율 노브 (0–100%)
  - 0% = 클린 DI 100%, 프로세스드 0%
  - 50% = 클린 DI 50%, 프로세스드 50% (동등 혼합)
  - 100% = 클린 DI 0%, 프로세스드 100%
- **Clean Level**: 클린 DI 경로 개별 레벨 트림 (−12 ~ +12 dB)
- **Processed Level**: 프로세스드 경로 개별 레벨 트림 (−12 ~ +12 dB)
- **IR Position** (2-way 선택):
  - **Post-IR** (기본): Blend → 캐비닛 IR 순서. 클린 DI는 캐비닛 컬러링 없이 원음 유지. 저역을 완전히 분리하고 싶을 때.
  - **Pre-IR**: Blend → 캐비닛 IR 순서가 아닌, Blend 결과물에 캐비닛 IR이 함께 적용. 클린 DI 포함 전체 혼합 신호가 캐비닛을 통과하므로 두 경로가 공간적으로 더 통일된 소리.

**출력 계산:**
```
mixed = (cleanDI × cleanLevel × (1 − blend)) + (processed × processedLevel × blend)

Post-IR: output = mixed                    ← 캐비닛 IR은 프로세스드 경로에서만 적용
Pre-IR:  output = cabinetIR(mixed)         ← 캐비닛 IR이 혼합 결과 전체에 적용
```
...
| 모노→스테레오 변환 | 신호 체인은 노이즈 게이트~파워앰프 구간 모노 처리. Cabinet IR 컨볼루션(스테레오 IR)에서 스테레오로 확장. Pre-IR 모드 시 DI Blend 후 캐비닛 통과 시점에서 변환 |
| 플러그인 지연 시간 보고 | `prepareToPlay()` 내에서 `setLatencySamples(oversamplingLatency + convolutionLatency)` 호출 필수 — DAW PDC(자동 지연 보상) 정확성을 위해 오버샘플링·컨볼루션 지연 합산값 보고 |
