...
- [ ] Standalone 실행 → Input Gain 노브 옆에 Passive/Active 토글 버튼 표시 확인 (수동 확인 필요)
- [ ] Active 버튼 선택 → 강조색(붉은 계열) 표시, Passive는 기본색 (수동 확인 필요)
- [ ] 베이스를 연결하고 Active 토글 ON → 소리가 약 10dB 작아짐 (액티브 베이스 과입력 방지 확인) (수동 확인 필요)
- [ ] Active → Passive 전환 → 소리 크기가 원래대로 복귀 (수동 확인 필요)
- [ ] Active 상태에서 프리셋 저장 → 재시작 후 Active 상태 유지 (수동 확인 필요)
...
- [ ] 팩토리 프리셋 로드 → 파라미터 값 즉시 반영 확인 (수동 확인 필요)
- [ ] 커스텀 파라미터 설정 → 저장 → Standalone 앱 종료 → 재시작 → 저장한 프리셋 불러오기 → 파라미터 값 일치 (수동 확인 필요)
- [ ] A 슬롯 저장 → 파라미터 변경 → A 슬롯 로드 → 원래값 복귀 (즉시 음색 변화 청취) (수동 확인 필요)
- [ ] B 슬롯에 다른 음색 저장 → A/B 전환 버튼 반복 클릭 → 두 음색 즉시 교체 (수동 확인 필요)
- [ ] .bmg 파일 Export → Import → 파라미터 일치 (수동 확인 필요)

---

## NoiseGate 동작 청취

- [ ] NoiseGate Threshold 최대(0dB) → 베이스를 약하게 치면 완전 무음 (수동 확인 필요)
- [ ] NoiseGate Threshold 최소(-60dB) → 모든 신호 통과, 배경 노이즈도 통과 (수동 확인 필요)
- [ ] Attack 1ms vs 50ms 차이 → 1ms는 게이트 오픈이 즉각적, 50ms는 느린 페이드인 (수동 확인 필요)
- [ ] NoiseGate Enabled OFF → Threshold/Attack/Release 설정 무관하게 신호 통과 (수동 확인 필요)

---

## Preamp 튜브 모델 차별화 청취

- [ ] 앰프 모델 American Vintage(Tube12AX7) 선택 → 따뜻하고 부드러운 포화음 청취 (수동 확인 필요)
- [ ] 앰프 모델 Modern Micro(JFETParallel) 선택 → 클린+그릿 혼합, Tube보다 날카로운 어택 (수동 확인 필요)
- [ ] 앰프 모델 Italian Clean(ClassD) 선택 → 왜곡 없음, 투명한 클린 톤 (수동 확인 필요)
- [ ] Input Gain 최대 → Tube12AX7 포화: NaN/Inf 없음, 크래시 없음 (수동 확인 필요)

---

## 튜너 동작 확인

- [ ] E1 현(41Hz) 연주 → 튜너 디스플레이에 "E" 표시, 센트 편차 바 움직임 (수동 확인 필요)
- [ ] A1 현(55Hz) 연주 → "A" 표시 확인 (수동 확인 필요)
- [ ] D2 현(73Hz) 연주 → "D" 표시 확인 (수동 확인 필요)
- [ ] G2 현(98Hz) 연주 → "G" 표시 확인 (수동 확인 필요)
- [ ] 무음 상태 → 튜너 피치 미감지, 크래시 없음 (수동 확인 필요)
- [ ] Tuner Mute ON → 완전 무음 출력 (수동 확인 필요)
- [ ] Tuner Mute OFF → 원래 소리 복귀 (수동 확인 필요)
...
- [ ] 앰프 모델 6종 전환 → 노브 레이아웃 변경, 음색 변화 확인 (수동 확인 필요)
...
- [ ] Bi-Amp ON + Crossover 200Hz → 저음 클린/고음 앰프처리 분리 청취 (수동 확인 필요)
...
- [ ] SettingsPage 열기 → 드라이버/장치/SR/버퍼 선택 가능 (수동 확인 필요)
