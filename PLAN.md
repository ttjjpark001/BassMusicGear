...
### ✅ CARRY
- **NoiseGateTest.cpp** (Phase 1 단위 테스트 누락) — `enabled=false` 신호 통과, Threshold 최대 신호 차단, Attack/Hold/Release 타이밍, NaN/Inf 없음
- **PreampTest.cpp** (Phase 1 단위 테스트 누락) — 4x 오버샘플링 앨리어싱 -60dBFS 이하, 모델별(12AX7/JFET/ClassDLinear) 게인 스테이징 차이, tanh 소프트 클리핑, NaN/Inf 없음
- **TunerTest.cpp** (Phase 3 단위 테스트 누락) — E1(41Hz)/A1(55Hz)/D2(73.4Hz) 피치 감지 ±5센트 이내, 음이름 변환 정확도(E/A/D/G), 무음 입력 크래시 없음
...
### 테스트 기준
- `Tests/NoiseGateTest.cpp` (Phase 1 CARRY)
  - enabled=false → 입출력 동일 (bypass)
  - Threshold 최대(-0dB) → 출력 RMS 거의 0 (신호 차단)
  - Threshold 최소(-60dB) → 신호 통과 (RMS 차이 없음)
  - Attack 1ms vs 50ms → 게이트 열리는 타이밍 차이 검증
  - NaN/Inf 없음 (0dBFS 입력)
- `Tests/PreampTest.cpp` (Phase 1 CARRY)
  - 4x 오버샘플링 적용 시 10kHz 이상 앨리어싱 -60dBFS 이하
  - 동일 입력에 대해 12AX7/JFET/ClassDLinear 3종 출력 RMS 서로 다름
  - 강한 드라이브(Gain=1.0)에서 출력이 ±1.5f 이내로 제한됨 (tanh 소프트 클리핑)
  - NaN/Inf 없음
- `Tests/TunerTest.cpp` (Phase 3 CARRY)
  - E1(41.2Hz) 사인파 입력 → 감지 주파수 ±5센트 이내
...
✅ CARRY 항목 먼저 처리 (단위 테스트 작성):
1. Tests/NoiseGateTest.cpp — enabled bypass, Threshold 최대/최소 신호 차단/통과, Attack 타이밍, NaN/Inf
2. Tests/PreampTest.cpp — 4x 오버샘플링 앨리어싱, 3종 모델 게인 차이, tanh 클리핑 진폭 제한, NaN/Inf
3. Tests/TunerTest.cpp — E1/A2 주파수 ±5센트, 음이름 변환(E/A/D/G), 무음 크래시 없음
Tests/CMakeLists.txt에 세 파일 추가 필수.
...
완료 기준: NoiseGateTest + PreampTest + TunerTest + PresetTest 전부 통과, 앱 재시작 후 프리셋 복원 확인, A/B 전환 동작 확인, EQ 사용자 프리셋 저장/불러오기/삭제 동작 확인.
...
- **OctaverTest.cpp** (Phase 4 단위 테스트 누락) — 서브옥타브 생성(82Hz→41Hz), 옥타브업 생성(82Hz→164Hz), dryLevel=1/subLevel=0 원음 통과, NaN/Inf 없음
- **EnvelopeFilterTest.cpp** (Phase 4 단위 테스트 누락) — enabled=false bypass, Up/Down direction 서로 다른 출력, Sensitivity 최대/최소 스윕 범위 차이, NaN/Inf 없음
...
### 테스트 기준
- `Tests/OctaverTest.cpp` (Phase 4 CARRY)
  - 82Hz 사인파 입력 + subLevel=1, dryLevel=0 → 출력에 41Hz 성분 존재
  - 82Hz 사인파 입력 + upLevel=1, dryLevel=0 → 출력에 164Hz 성분 존재
  - dryLevel=1, subLevel=0, upLevel=0 → 입출력 에너지 동일 (원음 통과)
  - NaN/Inf 없음 (고진폭 입력)
- `Tests/EnvelopeFilterTest.cpp` (Phase 4 CARRY)
  - enabled=false → 입출력 동일 (bypass)
  - Direction Up vs Down → 동일 입력에 대해 서로 다른 출력 스펙트럼
  - Sensitivity 최대 vs 최소 → 필터 스윕 개시 임계 레벨 차이
  - NaN/Inf 없음 (극단적 파라미터)
- `Tests/ChorusTest.cpp` (Phase 5 CARRY)
  - enabled=false → 입출력 동일 (bypass)
  - Mix=0.0 → 출력이 원음과 에너지 일치
  - Mix=1.0 + Rate/Depth 적용 → 출력 스펙트럼이 입력과 다름 (모듈레이션 확인)
  - NaN/Inf 없음
...
단위 테스트 작성 (Phase 4·5 CARRY):
1. Tests/OctaverTest.cpp — 서브옥타브/옥타브업 생성, dryLevel=1 원음 통과, NaN/Inf
2. Tests/EnvelopeFilterTest.cpp — bypass, Up/Down 출력 차이, Sensitivity 차이, NaN/Inf
3. Tests/ChorusTest.cpp — bypass, Mix=0 원음, Mix=1 모듈레이션 확인, NaN/Inf
Tests/CMakeLists.txt에 세 파일 추가 필수.
...
완료 기준: OctaverTest + EnvelopeFilterTest + ChorusTest + 전체 기존 테스트 그린 유지, 리사이즈/VUMeter/SignalChainView/색상 테마 동작 확인.
...
- **dirty 상태에서 프리셋 전환 시 저장 여부 확인**: dirty 표시(●)가 있는 상태에서 다른 프리셋으로 이동하거나 A/B 슬롯을 복원할 때, 현재 변경 내용을 저장할지 묻는 다이얼로그 표시를 고려.
  - dirty 표시 구현이 선행되어야 적용 가능.
  - 팩토리 프리셋은 저장 불가이므로 "다른 이름으로 저장" 옵션 또는 묻지 않고 이동 중 선택 필요.
  - DAW 플러그인 환경에서는 호스트가 파라미터 변경을 추적하므로 생략 가능성도 검토.
...
| Phase 5 | ChorusTest.cpp 단위 테스트 누락 | Phase 9 | ☐ |
