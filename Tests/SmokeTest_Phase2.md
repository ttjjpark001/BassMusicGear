...
- [ ] American Vintage: Mid Position 스위치 5단계 전환 시 중역 중심 주파수 변화 청취 (수동 확인 필요)
- [ ] British Stack: Bass 노브 최대/최소 변경 시 고역(8kHz 이상)이 거의 변하지 않음 (수동 확인 필요)
  - 단위 테스트로 검증됨: Bass 변경 시 8kHz 변화량 2dB 이내
- [ ] Modern Micro: Grunt 최대 → 저중역(250Hz) 부스트 청취 (수동 확인 필요)
- [ ] Italian Clean VPF 최대 → 380Hz 미드 스쿱(노치), 35Hz 서브 부스트, 10kHz 고음 부스트 확인 (수동 확인 필요)
  - 단위 테스트로 검증됨: 1kHz 대비 380Hz 감쇠 6dB 이상
- [ ] Italian Clean VLE 최대 → 고역 롤오프 확인 (소리가 어두워짐, 8kHz 크게 감쇠) (수동 확인 필요)
...
- [ ] American Vintage/Tweed Bass/British Stack: Tube12AX7 프리앰프 — 드라이브 시 짝수 고조파 왜곡(따뜻한 톤) 청취 (수동 확인 필요)
- [ ] Modern Micro: JFET 병렬 구조 — 클린 톤 유지하면서 드라이브 시 미세한 그릿 추가 청취 (수동 확인 필요)
  - 단위 테스트로 검증됨: JFET 출력 신호 무결성(NaN/Inf 없음, RMS 유효 범위)
- [ ] Italian Clean: ClassDLinear — 드라이브를 올려도 왜곡 없는 클린 톤 유지 (수동 확인 필요)
