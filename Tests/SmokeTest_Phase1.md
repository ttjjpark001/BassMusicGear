...
- [x] Standalone 실행 → 창 표시 확인 (Phase 3 테스트 중 확인 완료)
- [x] 앱 정상 종료 → 크래시 없음 (Phase 3 테스트 중 확인 완료)

---

## 오디오 입출력

- [x] 오디오 인터페이스 연결 → 입력 신호 감지 (Phase 3 튜너 테스트 중 베이스 연주로 확인 완료)
- [x] 베이스 기타 연주 → 처리된 소리 출력 (Phase 3 튜너 테스트 중 확인 완료)
- [ ] ⚠️ 무입력 상태 → 노이즈 게이트 동작, 무음 또는 매우 작은 출력 (Phase 8 UI 구현 후 확인 필요 — NoiseGate UI 없어 현재 불가)
...
- [x] Bass 노브 최대 → 저역 증가 청취 확인 (확인 완료)
- [x] Treble 노브 최대 → 고역 증가 청취 확인 (확인 완료)
- [x] Mid 노브 최소 → 미드 스쿱 청취 확인 (확인 완료)
- [x] 세 노브 중간(0.5) → 전대역 비교적 균일한 응답 (확인 완료)

---

## 파워앰프

- [x] Drive 노브 증가 → 포화감(새추레이션) 증가 청취 (확인 완료)
- [x] Presence 노브 증가 → 고역 공기감 밝기 증가 청취 (확인 완료)

---

## 캐비닛 시뮬레이션

- [x] Cabinet 바이패스 OFF → 캐비닛 IR 컬러링 포함 소리 출력 (확인 완료)
- [x] Cabinet 바이패스 ON → 캐비닛 없이 더 평탄한 DI 소리 출력 (확인 완료)
